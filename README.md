...
<img src="./img/sample_play.gif" width="25%"/></br>

최종 발표 Youtube 영상 [링크](https://youtu.be/VQuRIGhYJnk)
...
- 고정된 장애물은 서로 만나지 않으며, <b>오른쪽 장애물과 왼쪽 장애물은 항상 3.2m만큼 떨어져 있다</b>.</br>
...
### 2.1.2. 참고한 것
- [Godot](https://godotengine.org/) 또는 [Unity](https://unity.com/)의 UI 시스템을 모방함.
...
#### 3.1.1. Title
- 메뉴 버튼들을 화면에 보여줍니다.
- 메뉴 버튼에는 `게임 시작`, `게임 종료`가 있습니다.
- `게임 시작` 버튼을 누른 경우 게임 화면에 진입합니다.
- `게임 종료` 버튼을 누른 경우 애플리케이션을 종료합니다.
- 최고 기록과 현재 보유중인 `좋아요`의 갯수를 보여준다.

#### 3.1.2. Prepare
- 티노가 화면 위쪽에서 천천히 내려오면서 게임이 시작되려 함을 사용자에게 보인다.

#### 3.1.3 Boost
- 게임 시작 전에 일정 좋아요를 지불하여 `에너지 음료` 아이템을 사용할 수 있게 한다.
...
#### 5주차 (05/03~05/09)
- [x] 플레이어의 Dive 상태를 구현 (이월)
- [x] 낙하산의 내구도가 0이하일 경우 플레이어 상태를 Dive 상태로 변경 (이월)
- [x] 장애물 오브젝트를 화면에 출력
...
#### 7주차 (05/17~05/23)
- [x] 화면 맨 위쪽 왼쪽에 낙하산의 내구도를 출력 (이월)
- [x] 화면 맨 위쪽 오른쪽에 획득한 좋아요의 갯수를 출력 (이월)
- [ ] ~~InGamePrepare 장면, InGameFinish 장면을 구현~~ (실패)
- [x] InGameFinish 장면에서 상황에 따라 플레이어 애니메이션을 다르게 출력
- [x] 플레이어와 장애물 오브젝트가 충돌할 경우 InGameFinish 장면으로 전환
- [ ] ~~InGamePrepare 장면에서 InGame 장면으로 전환~~ (실패)
- [ ] ~~Title 장면을 구현~~ (실패)
- [ ] ~~Title 장면에서 게임 시작 버튼을 누를 경우 InGamePrepare 상태로 전환~~ (실패)
- [ ] ~~Intro 장면을 구현~~ (_중단_)
- [ ] ~~Intro 장면에서 Title 장면으로 전환~~ (_중단_)

#### 8주차 (05/24~05/30)
- [x] FinishGameScene에서 최고 기록을 출력하도록 구현 (세분화)
- [x] PrepareGameScene 장면을 구현 (이월)
- [x] PrepareGameScene 장면에서 InGame 장면으로 전환 구현 (이월)
- [ ] ~~게임 최고기록을 저장하는 기능을 구현~~ (실패)

#### 9주차 (05/31~06/06)
- [x] 게임 최고기록을 저장하는 기능을 구현 (이월)
- [x] Title 장면을 구현 (이월)
- [x] Title 장면에서 InGamePrepare 장면으로 전환 구현 (이월)
- [x] ResultGameScene 장면을 생성
- [x] ResultGameScene 장면에서 결과를 출력하도록 구현
- [x] 게임 배경음악 출력
- [x] 아이템을 먹었을 때 효과음 출력
- [ ] ~~Title 장면에 장비 업그레이드 기능 구현~~ (기획 수정)
- [x] 게임 시작 전에 좋아요를 지불하여 무적 아이템을 사용할 수 있도록 구현

### 4.1. 주차별 Commit 횟수 및 개발 일정 (결과)

<img src="./img/commits.jpg" width="80%"/>

### 4.2. 변경된 기획

#### 이전 계획
<img src="./img/prev_plan.jpeg" width="50%"/> 

차고에서 일정 `좋아요`를 지불하여 `낙하산`을 강화하게 한다.

<b>발생한 문제점</b>
- `낙하산`을 강화할 경우 게임 재미가 떨어짐.
- `낙하산` 강화에는 한계가 존재하기 때문에 `좋아요`를 계속 소비하지 못함.

#### 수정 후 계획
<img src="./img/curr_plan.jpeg" width="50%"/>

게임 시작 전 일정 `좋아요`를 지불하여 게임 시작과 동시에 `에너지` 아이템을 사용할 수 있도록 한다.
...
## 6. 수업에 대해서
안드로이드 시스템과 게임 시스템 설계를 살펴볼 수 있는 점은 좋았으나, 게임에서 `Java`를 사용하지 않기에 `Android NDK`를 사용할지 기대를 했었고, 안드로이드의 `Activity Lifecycle`에 대해 더 자세히 가르칠 필요가 있다고 생각한다. 결과적으론 좋은 수업이었으나, 자신만의 프레임워크가 확립된 4학년때 배우기 보단 2~3학년 사이에 수업을 들었으면 더 좋았을 것 같다.
